...
n.comps <- c(2,12,4,14,6,16,8,18,10)
...
for(ncomps in n.comps){
    ...
    save(result, file=paste("../SOM/som_forg2stat/result", ncomps, "comps.f2s_logtime.RData", sep="")) # Save the result for each number of clusters	  # specified in n.comps.
}

## Plot the BIC curve to choose the number of clusters.
BICF2S <- c()
for(i in 1:length(n.comps)){
    load(paste("../SOM/som_forg2stat/result", n.comps[i], "comps.f2s_logtime.RData", sep=""))
    BICF2S[i] <- result[[1]]$BIC
    print(paste(n.comps[i], "-->", BICF2S[i], sep=" "))
}
pdf(paste("../SOM/som_forg2stat/BIC_results.f2s.pdf", sep=""))
plot(n.comps, BICF2S, type="b", xlab="G", ylab="BIC")
dev.off()
## Plot the clusters for the best result (as chosen by BIC)
## For example, to choose the solution for 6 clusters, set best_G to 6.

best_G <- 76
best_file <- paste("../SOM/som_forg2stat/result", best_G, "comps.f2s_logtime.RData", sep="")
load(best_file)
plotResults(result[[1]], data=full.data, paste("results_",best_G,"_clust.F2S", sep=""))